    ...
    const geometry_msgs::msg::Pose& center_pose, double radius, int num_points = 36)
{
    ...
    for (int i = 0; i < num_points; ++i)
    {
        ...
        geometry_msgs::msg::Pose waypoint = center_pose;  // 拷贝，不会修改 const 引用
        waypoint.position.x += radius * std::cos(angle) - 0.1;
        waypoint.position.y += radius * std::sin(angle);
        waypoint.position.z -= 0.14;
...
void draw_gcode(const rclcpp::Node::SharedPtr& node, const std::vector<double>& init_pose)
{
    ...
    // 5.生成圆形路径点
    std::vector<geometry_msgs::msg::Pose> waypoints = generateCircleWaypoints(current_pose, 0.05, 36); // 半径为0.1，点数为36
