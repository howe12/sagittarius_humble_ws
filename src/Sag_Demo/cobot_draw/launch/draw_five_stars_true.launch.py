...
def launch_setup(context, *args, **kwargs):
    ...
    # 2. 设定moveit_config & moveit_controllers
    moveit_config = (
    ...
    # 4. 导入gcode文件，可以确定机械臂的初始位置以及抬笔高度等
    ...
    # 5. 绘制程序，通过延迟设定等待机械臂驱动启动
    draw_gcode = Node(
        package='cobot_draw',
        executable='draw_five_stars',
        name='draw_five_stars',
        output='screen',
        parameters=[
            gcode_config_params,
    ...
    delayed_draw_gcode = TimerAction( # 延时启动 draw_gcode 节点（延时 5 秒）
        period=5.0,  
        actions=[draw_gcode],
    )
