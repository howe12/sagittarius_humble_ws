...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        sag_cam_calibration_dir = sys.path[0]
        usb_cam_config_file = os.path.join(sag_cam_calibration_dir, '../../share/sagittarius_cam_calibration/config', 'usb_cam_calibration.yaml')
        # add ground plane
        ...
        # initialize the GUI
        self.initUI()
        self.upload_application_parameters(usb_cam_config_file)
    ...
    def upload_application_parameters(self,origin_file=None):
    ...
    def save_image(self, img, index):
        ...
        self.logger.info(f'save_image:{filename}')
        cv2.imwrite(filename, img)
    def run_take_samples(self):
        self.logger.info('run_take_samples')
        ...
        # 添加检测图像中是否有标定板
        img, charuco_corners, charuco_ids, img_size = self.detect_charuco_board(img)
        # 如果没有检测到标定板，返回
        if charuco_corners is None:
            self.logger.info("No charuco board found!")
            return
        # 输出检测到的标定板
        self.logger.info(f"Found {len(charuco_ids)} markers!")
        
        gripper_pose = self.gripper_pose()
        self.logger.info(f'gripper_pose:{gripper_pose}')
