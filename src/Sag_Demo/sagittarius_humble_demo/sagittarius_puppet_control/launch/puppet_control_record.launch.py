...
def launch_setup(context, *args, **kwargs):
    ...
    # bag_path = sagittarius_puppet_control + '/bag/' + robot_model.perform(context) + '_bag_' + current_time
    ...
    # 使用 ExecuteProcess 启动 ros2 bag record
    rosbag_record_process = TimerAction(
        period=5.0,  # 延迟时间（秒）
        actions=[
            LogInfo(msg=["Bag path is: ", bag_path]),
            LogInfo(msg=["Joint command topic: /", robot_name_launch_arg.perform(context), "/joint/commands"]),
            LogInfo(msg=["Gripper command topic: /", robot_name_launch_arg.perform(context), "/gripper/command"]),

            ExecuteProcess(
                cmd=[
                    "ros2", "bag", "record",
                    "-o", [bag_path],
                    f"/{robot_name_launch_arg.perform(context)}/joint/commands",
                    f"/{robot_name_launch_arg.perform(context)}/gripper/command",
                ],
                output="screen"
            )
        ]
