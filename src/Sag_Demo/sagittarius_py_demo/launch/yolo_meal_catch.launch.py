...
def launch_setup(context, *args, **kwargs):
    ...
    yolo_detect = Node(
        package='ros2_yolov8',
        executable='meal_predict_tf',
        parameters=[
            # 设置相机话题名称
            {'image_topic': image_topic,
             'image_info_topic': '/camera1/camera_info',},
        ],  
    )

    # 4.1 延时启动 yolo_detect 节点（延时 5 秒）
    delayed_yolo_detect = TimerAction(
        period=10.0,  # 延时 10 秒启动
        actions=[yolo_detect],
    )
    ...
    rviz2 = Node(
        ...
        executable='rviz2',
        name='rviz2',
        arguments=['-d', PathJoinSubstitution([FindPackageShare("sagittarius_py_demo"), 'rviz', 'meal_rviz.rviz'])],
        parameters=[
