...
class ImgShow(Node):
    ...
    def __init__(self):
        ...
        # 从launch文件中确定订阅的相机话题名称
        self.image_topic = self.declare_parameter('image_topic', '/camera/camera/color/image_raw').value
        ...
        self.get_logger().info("********========     1,在显示的图片上,选取想要识别的颜色;                  ========******** ")
        self.get_logger().info("********========     2,选取后暂时以字典保存,待全部颜色识别完后,按s进行保存;========********")
        self.get_logger().info("********========     3,r代表红色,g代表绿色,b代表蓝色,y代表黄色;            ========********")
        self.get_logger().info("********========     4,q代表刷新,s代表保存.                                ========********")
        self.get_logger().info("********===========================================================================********")
        ...
        sagittarius_py_demo = get_package_share_directory('sagittarius_py_demo')
        self.hsv_file_path = os.path.join(sagittarius_py_demo,'params', 'hsv_values.txt')  # hsv文件地址
    ...
    def image_callback(self, img): 
        ...
        if c == 114 or c == 103 or c == 98 or c == 121:
            ...
            self.get_logger().info(f"================     🎯当前hsv_dict字典内容为{self.hsv_dict}")

        elif c == 115:
            self.get_logger().info('保存全部字典')
    ...
    def mouse_callback(self, event, x, y, flags, param):
        ...
        # 获取鼠标左键松开事件
            ...
            self.get_logger().info(f"================     检测到的[h,s,v]为{self.roi_h},{self.roi_s},{self.roi_v}")
            self.get_logger().info(f"================     观察mask图片,如果满意，在图片窗口输入颜色对应英文的首字母，如'r','g','b','y'")
            ...
            if roi_num > 0:
                ...
                self.get_logger().info("请在BGR图像上拖拽选取颜色")
    ...
    def save_hsv_values_to_file(self):
        ...
        self.get_logger().info("HSV values saved to leo_agv/install/leo_grasp/share/leo_grasp/param/hsv_values.txt")
