...
class RobotMoveitAction(Node):
    ...
    def get_trans(self):
        # 定义抓取的颜色名称
        color_item = ["red","yellow","green","blue"]
        # 循环抓取颜色
        for i in range(len(color_item)):
            ...
            self.transform_stamped = None
            try:
                self.transform_stamped = self.tf_buffer.lookup_transform("sgr532/base_link", str(color_item[i])+"_object",Time())
                self.grasp_flag = True
                color_name = color_item[i]
                # self.get_logger().info(f"🎯 转换信息为: {self.transform_stamped}")
                self.get_logger().info(f"🎨 抓取颜色为: {color_name}")
                # self.task()
                # self.transform_stamped = None
                return True, color_name,self.transform_stamped   
    ...
    def motion(self):
        # while rclpy.ok():
        flag, color_name,trans = self.get_trans()
        ...
        if flag:
            ...
            self.ik_motion(trans_x, trans_y, trans_z+0.02  , rot_x, rot_y, rot_z, rot_w)  # 机械臂移动到物体上方
            time.sleep(1)
            self.ik_motion(trans_x, trans_y, trans_z-0.01  , rot_x, rot_y, rot_z, rot_w)       # 机械臂向下移动
            time.sleep(1)
