...
class RobotMoveitAction(Node):
    def __init__(self):
        ...
        # 机械臂检测状态
        self.detect_values = {"joint1": 0.0,"joint2": 1.4067353771074296,"joint3": -0.3560471674068432,"joint4": 0.0,"joint5": -1.799434458806154,"joint6": 0.0017453292519943296,}
        # 机械臂放置状态
        # 使用字典的方式存放机械臂的各个关节位置
        self.place_positions = {
            "place0": {"joint1": 1.98967, "joint2": 0.7470, "joint3": -0.2914, "joint4": 0.0, "joint5": -1.7016, "joint6": -0.0035},
            "place1": {"joint1": 1.46782, "joint2": 0.7470, "joint3": -0.2914, "joint4": 0.0, "joint5": -1.7016, "joint6": -0.0035},
    ...
    def motion(self):
        ...
        if flag:
            ...
            self.ik_motion(trans_x, trans_y, trans_z+0.03  , rot_x, rot_y, rot_z, rot_w)  # 机械臂移动到物体上方
            time.sleep(1)
            self.ik_motion(trans_x, trans_y, trans_z  , rot_x, rot_y, rot_z, rot_w)  # 机械臂向下移动
            time.sleep(1)
            self.gripper_fk_motion(self.gripper_close_values)
            time.sleep(2)
            self.ik_motion(trans_x, trans_y, trans_z+0.12  , rot_x, rot_y, rot_z, rot_w)  # 机械臂向上移动
            time.sleep(2)
