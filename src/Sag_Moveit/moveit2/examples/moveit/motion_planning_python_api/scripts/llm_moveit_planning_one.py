...
class LLM_motion(Node):
    ...
    def adjust_position_and_orientation(self,translation_delta=None, rotation_delta=None):
        ...
        if rotation_delta:
            for axis, angle in rotation_delta.items():
                ...
                # 根据轴选择合适的旋转矩阵
                if axis == 'x':
                    rot_mat_delta = R.from_euler('x', angle_rad).as_matrix()[:3, :3]
                elif axis == 'y':
                    rot_mat_delta = R.from_euler('y', angle_rad).as_matrix()[:3, :3]
                elif axis == 'z':
                    rot_mat_delta = R.from_euler('z', angle_rad).as_matrix()[:3, :3]
                
                # 更新旋转矩阵
                rotation_mat = np.dot(rotation_mat, rot_mat_delta)
        ...
        # 将平移向量放入 4x4 变换矩阵的最后一列（前三个元素）
        transform_matrix[:3, 3] = np.array([translation_delta['x'], 
                                            translation_delta['y'], 
                                            translation_delta['z']])

        transform_stamped = self.matrix_to_transform_stamped(transform_matrix,"tool0","target")
        ...
        pose_stamped_msg = self.transform_stamped_to_pose_stamped(transform_stamped)
    ...
    def run_task(self):
        ...
        # 基于末端坐标系
        translation_delta = {'x': 0.0,'y': 0.10,'z': 0.0}  # 单位为米，x增加表示向左，y增加表示向前，z增加表示向下
